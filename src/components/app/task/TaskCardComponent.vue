  ...
  <w-flex fill-height >
    <w-flex align-center justify-space-between class="news-card blue-grey-dark3--bg bdrs3">
      ...
      <div class="d-flex justify-start pa2" style="flex-direction: column; width: calc(100% - 200px)">
        <div class="blue-grey-light4 title2">{{ task.author }} - {{ task.title }}</div>
        <div class="blue-grey-light4 title3 text-italic text-muted book-description"> {{ task.description }}</div>
      </div>
      <w-divider class="my4"  vertical></w-divider>
        ...
        <w-tooltip left color="blue-grey-light4" bg-color="blue-grey-dark5" v-if="!user.isAdmin">
          <template #activator="{ on }">
            <w-button
                ...
                :color="task.isStudentBook?'teal':'blue-grey-light4'"
                @click="task.isStudentBook?destroySaveTasks(task):storeSaveTasks(task)"
                lg
            >
            </w-button>
          </template>
          <div class="title3">
            {{ task.isStudentBook?'Книга збережена':'Зберегти книгу' }}
          </div>
    ...
    >
      ...
      <w-flex column style="padding: 24px">
        <div class="title2 text-center mb4">{{ task.author }} - {{ task.title }}</div>
        <div class="title3 text-italic book-description1"> {{ task.description }}</div>
...
<!--      <template #title>-->
<!--        <div class="body blue-grey-light3">{{ task.author }}</div>-->
...
<!--                    :color="task.document.isSaveDocument?'cyan-dark4':'blue-grey-dark4'"-->
<!--                    @click="task.document.isSaveDocument?destroySaveTasks(task):storeSaveTasks(task)"-->
<!--                    xl-->
<!--                >-->
...
<!--              </template>-->
<!--              <div class="body">-->
<!--                {{ task.document.isSaveDocument?'Видалити з кабінету':'Зберегти в кабінеті' }}-->
<!--              </div>-->
