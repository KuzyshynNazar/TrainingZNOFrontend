  ...
  <w-flex fill-height >
    <w-flex align-center justify-space-between class="news-card blue-grey-dark3--bg bdrs3">
      <div class="d-flex align-center justify-center pa2" style="width: 100px">
        <w-image :src="require('../../../assets/test-sample.png')" tag="img" style="max-width: 60px"></w-image>
      </div>
      ...
      <div class="d-flex justify-start pa2" style="flex-direction: column; width: calc(100% - 200px)">
        <div class="blue-grey-light4 title2">{{ testSample.author }} - {{ testSample.title }}</div>
        <div class="blue-grey-light4 title3 text-italic text-muted book-description"> {{ testSample.description }}</div>
      </div>
      <w-divider class="my4"  vertical></w-divider>
        ...
        <w-tooltip left color="blue-grey-light4" bg-color="blue-grey-dark5" v-if="!user.isAdmin">
          <template #activator="{ on }">
            <w-button
                ...
                :color="testSample.isStudentBook?'teal':'blue-grey-light4'"
                @click="testSample.isStudentBook?destroySaveTestSamples(testSample):storeSaveTestSamples(testSample)"
                lg
            >
            </w-button>
          </template>
          <div class="title3">
            {{ testSample.isStudentBook?'Книга збережена':'Зберегти книгу' }}
          </div>
    ...
    >
      ...
      <w-flex column style="padding: 24px">
        <div class="title2 text-center mb4">{{ testSample.author }} - {{ testSample.title }}</div>
        <div class="title3 text-italic book-description1"> {{ testSample.description }}</div>
  ...
  methods:{
    ...
    destroySaveTestSamples(testSample){
      this.deleteSavedTestSample(testSample.id).then(()=>{
        testSample.isStudentBook=false
