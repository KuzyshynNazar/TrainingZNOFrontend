...
const routes = {
    get: {
        ...
        taskTemplate: `${origin}/taskTemplates/{id}`,
    },
    post: {
        create: `${origin}/taskTemplates`,
    },
    put: {
        update: `${origin}/taskTemplates/{id}`,
    },
    delete: {
        destroy: `${origin}/taskTemplates/{id}`
    }
};
...
exports.show = (id, headers) => {
    const url = routes.get.taskTemplate.replace('{id}', id);
    return axios.get(url, headers);
};
