...
<!--      </div>-->
<!--    </w-grid>-->
    <w-card style="max-width: calc(100vw - 275px); max-height: calc(100vh - 106px);  overflow-y: auto" bg-color="blue-grey-dark1" content-class="pa0" no-border>
      <w-table
          :headers="tableStudentBookHeader"
          :items="filteredFormulaList"
          fixed-headers
          style="width: 100%;"
      >
        <template #item="{ item, classes,index  }">
          <tr :class="classes">
            <td class="pl1 pa1 blue-grey-light5">
              <w-image :src="require('../../assets/formulas.png')" tag="img" style="max-width: 50px"></w-image>
            </td>
                ...
                <w-tooltip left bg-color="blue-grey-dark3" color="blue-grey-light4">
                  <template #activator="{ on }">
                    <w-button
                        ...
                        color="blue-grey-light5"
                        @click="destroySaveFormulas(item)"
                        lg
                    >
        ...
        <template #no-data>
          <div v-if="saveFormulas.length!==0">
            <div class="title2">
              Нічого не знайдено!
            </div>
            <div>
