...
<!--        gap="3"-->
<!--        class="pa3"-->
<!--        v-if="filteredTestSampleList!==undefined && filteredTestSampleList.length!==0"-->
<!--    >-->
<!--      <div v-for="(testSample, index) in filteredTestSampleList" :key="index" class="sh1">-->
<!--        <TestSampleCardComponent-->
...
<!--    </w-grid>-->
    <w-card style="max-width: calc(100vw - 275px); max-height: calc(100vh - 106px);  overflow-y: auto"
      ...
      >
        <template #item="{ item, classes,index  }">
          <tr :class="classes">
            <td class="pl1 pa1 blue-grey-light5">
              <w-image :src="require('../../assets/test-sample.png')" tag="img" style="max-width: 50px"></w-image>
            </td>
                ...
                <w-tooltip left bg-color="blue-grey-dark3" color="blue-grey-light4">
                  <template #activator="{ on }">
                    <w-button
                        ...
                        color="blue-grey-light5"
                        @click="destroySaveTestSamples(item)"
                        lg
                    >
        ...
        <template #no-data>
          <div v-if="saveTestSamples.length!==0">
            <div class="title2">
              Нічого не знайдено!
            </div>
            <div>
  ...
  <w-button absolute top left bg-color="blue-grey-dark3" color="blue-grey-light3" class="mt3" style="top:40px; left: 250px"
            icon="mdi mdi-magnify" xl tile @click="openDrawer">
...
// import TestSampleCardComponent from "@/components/student/account/TestSampleCardComponent";
const URL_STORAGE = process.env.VUE_APP_URL_STORAGE
const BASE_URL = process.env.VUE_APP_API_URL
export default {
  name: 'MyFormulaPage',
  // components: {TestSampleCardComponent},
  data() {
    return {
  ...
  computed: {
    ...
    filteredTestSampleList() {
      return this.saveTestSamples.filter(testSample => {
  ...
  methods: {
    ...mapActions({
      savedTestSamples: 'saveTestSamples/savedTestSamples',
