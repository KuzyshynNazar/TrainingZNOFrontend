...
<!--    </w-grid>-->
    <w-card style="max-width: calc(100vw - 275px); max-height: calc(100vh - 106px);  overflow-y: auto"
      ...
      <w-table
          ...
          :items="filteredTaskTemplateList"
          fixed-headers
          style="width: 100%;"
      >
        <template #item="{ item, classes,index  }">
          <tr :class="classes">
            <td class="pl1 pa1 blue-grey-light5">
              <w-image :src="require('../../assets/task-template.png')" tag="img" style="max-width: 50px"></w-image>
            </td>
                ...
                <w-tooltip left bg-color="blue-grey-dark3" color="blue-grey-light4">
                  <template #activator="{ on }">
                    <w-button
                        ...
                        color="blue-grey-light5"
                        @click="destroySaveTaskTemplates(item)"
                        lg
                    >
        ...
        <template #no-data>
          <div v-if="saveTaskTemplates.length!==0">
            <div class="title2">
              Нічого не знайдено!
            </div>
            <div>
