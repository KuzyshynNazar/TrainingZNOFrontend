<template>
  <w-flex justify-space-between style="width: 100%; padding-left: 290px" column fill-height class="pr3 pt3 pb3 pl10">
    ...
    <!--    >-->
    <!--      <div v-for="(book, index) in filteredBookList" :key="index" class="sh1">-->
    <!--        <BookCardComponent-->
    <!--            :book="book"-->
    ...
    <w-card style="max-width: calc(100vw - 275px); max-height: calc(100vh - 106px);  overflow-y: auto"
      ...
      >
        <template #item="{ item, classes,index  }">
          <tr :class="classes">
            <td class="pl1 pa1 blue-grey-light5">
              <w-image :src="require('../../assets/book.png')" tag="img" style="max-width: 50px"></w-image>
            </td>
        ...
        <template #no-data>
          <div v-if="saveBooks.length!==0">
            <div class="title2">
              Нічого не знайдено!
            </div>
            <div>
              <w-button @click="search=''" class="pa4 mt2" lg bg-color="blue-grey-dark3" color="blue-grey-light4"
                        icon="mdi mdi-reload"
              ></w-button>
    ...
    <w-drawer
        ...
        height="50px"
        color="blue-grey-dark3" bg-color="blue-grey-light4"
    >
      <w-flex class="mx10">
        <w-input
            style="width: 100%"
  ...
  <w-button absolute top left bg-color="blue-grey-dark3" color="blue-grey-light4" class="mt3"
...
// import BookCardComponent from "@/components/student/account/BookCardComponent";
...
export default {
  ...
  computed: {
    ...
    filteredBookList() {
      return this.saveBooks.filter(book => {
  ...
  methods: {
    ...mapActions({
      savedBooks: 'saveBooks/savedBooks',
      saveBook: 'saveBooks/saveBook',
      deleteSavedBook: 'saveBooks/deleteSavedBook',
    }),
    openDrawer() {
