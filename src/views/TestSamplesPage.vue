<template>
  ...
  >
    <div v-for="(testSample, index) in filteredTestSampleList" :key="index" class="sh2 bdrs3">
  ...
  <w-image :src="require('./../assets/math-logo.png')" tag="img" style="max-width: 10vw; position: fixed; top:40px; left: 10px;  transform: rotate(-180deg) scale(-1, 1);" ></w-image>
...
import TestSampleCardComponent from "@/components/app/testSample/TestSampleCardComponent";
...
export default {
  ...
  computed: {
    ...mapGetters({
      testSamples: 'appTestSamples/testSamples',
    }),
    filteredTestSampleList() {
      return this.testSamples.filter(testSample => {
  ...
  methods: {
    ...mapActions({
      getAllTestSamples: 'appTestSamples/getAll',
